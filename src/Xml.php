...
class Xml
{
     ...
     * @throws \DOMException
     */
    public static function build(array|string|object $input, array $options = []): SimpleXMLElement|DOMDocument
    {
        $defaults = [
            'return'       => 'simplexml',
            'loadEntities' => false,
    ...
    {
        if (!($input = trim($input)))
            throw new XmlException('Empty string cannot be loaded.');
        ...
        try {
            ...
            throw new XmlException('Xml cannot be read. ' . $e->getMessage(), (int)$e->getCode(), $e);
        } finally {
            if ($hasDisable && !$options['loadEntities']) {
     ...
     */
    public static function loadHtml(string $input, array $options = []): SimpleXMLElement|DOMDocument|null
    {
        $defaults = [
            'return'       => 'simplexml',
            'loadEntities' => false,
        ...
        if (!($input = trim($input)))
            throw new XmlException('Empty string cannot be read.');
        ...
        foreach ($data as $key => $value) {
            if (is_string($key)) {
                ...
                if (!is_array($value)) {
                    ...
                    if ($isNamespace !== false && $node instanceof \DOMElement) {
...
// Struct
    ...
    protected static function _toArray(SimpleXMLElement $xml, array &$parentData, string $ns, array $namespaces): void
    {
        ...
        foreach ($namespaces as $namespace) {
            if ($attributes = $xml->attributes($namespace, true)) {
